...
//main root
const mainroot = app.database().ref().child('app').child('panels').child('information');
var headd;
var bodyy;
...
class Multifunctionall  extends Component {
    ...
    constructor(props){
        super(props);
        this.mainroot= app.database().ref().child('app').child('panels').child('information');
        this.root = app.database().ref().child('app').child('panels').child('information').child('items');
        this.state = {
            headd : headd,
            bodyy: bodyy,
            information:[],
    ...
    componentWillMount() {
        const  pinformation=this.state.information ;
        ...
        this.root.on('child_added', snap => {
            ...
            //comment
            const comm = app.database().ref().child('app').child('panels').child('information').child('items').child(snap.key).child('comment group');
            const revise=[] ;
            ...
            // star
            const rom = app.database().ref().child('app').child('panels').child('information').child('items').child(snap.key).child('rate');
            var i=0;
            var rate=0;
            var orate=0;
    ...
    // enter for comment
    rchange = (id, value) => {
        const rootstar = app.database().ref().child('app').child('panels').child('information').child('items');
    ...
    render() {
        ...
        return (
            <Panel className="table">
                ...
                {this.state.information.map((panel) =>{
                    ...
                    return(
                        <div key={panel.id}>
                            <ListGroupItem> <a className="items" href={panel.link}> {panel.name} </a>
                                ...
                                <Commentt onPressEnterr={panel.id} lm={panel.revises} addr={"information"} panel={"panels"} />
